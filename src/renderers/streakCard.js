...
export function renderStreakCard(data = {}, themeName = 'dark', options = {}) {
  ...
  const streakRange = streakStats.streakRange || '';
  const currentStreakRange = streakStats.currentStreakRange || (currentStreak > 0 ? 'Active' : 'No Active Streak');
  const longestStreakRange = streakStats.longestStreakRange || '';
...
>
  <style>
    .stat-number {
      font: 700 28px 'Segoe UI', Ubuntu, Sans-Serif, -apple-system;
  ...
  <g class="fade-in" style="animation-delay: 150ms" transform="translate(82.5, 45)">
    ...
    <text x="0" y="45" text-anchor="middle" class="stat-number">${formatNumber(totalContributions)}</text>
    <text x="0" y="70" text-anchor="middle" class="stat-label">Total Contributions</text>
    <text x="0" y="90" text-anchor="middle" class="stat-range">${escapeXml(streakRange)}</text>
  </g>

  <!-- Block 2: Current Streak (Highlighted) -->
  <g class="fade-in" style="animation-delay: 300ms" transform="translate(247.5, 40)">
    <!-- Animated Flame Icon -->
    ...
    <text x="0" y="50" text-anchor="middle" class="stat-number">${formatNumber(currentStreak)}</text>
    <text x="0" y="75" text-anchor="middle" class="stat-label">Current Streak</text>
    <text x="0" y="95" text-anchor="middle" class="stat-range">${escapeXml(currentStreakRange)}</text>
  </g>

  <!-- Block 3: Longest Streak -->
  <g class="fade-in" style="animation-delay: 450ms" transform="translate(412.5, 45)">
    <!-- Crown / Zap Icon -->
    <svg x="-10" y="0" fill="${theme.icon_color}" height="20" width="20" viewBox="0 0 16 16">
    ...
    <text x="0" y="45" text-anchor="middle" class="stat-number">${formatNumber(longestStreak)}</text>
    <text x="0" y="70" text-anchor="middle" class="stat-label">Longest Streak</text>
    <text x="0" y="90" text-anchor="middle" class="stat-range">${escapeXml(longestStreakRange || streakRange)}</text>
  </g>
</svg>`;
}
